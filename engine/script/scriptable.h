...
{
	// Based on entt poly crash course => allows to query different Scriptables using Scriptable in the view creation (I hope...)
	...
	struct __Scriptable : entt::type_list<> 
	{
		...
		struct type : Base
		{
			...
			// and the methods for the concept simply invoke the actual implementation passing the arguments
		};
	...
	// at this point use entt::poly to wrap the compatible classes; this will be the class used to add the component
